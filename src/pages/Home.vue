<template>
	<div id="main" class="app">
		<div class="divHead">
			<img src="../assets/user.png" @click="toUserPage" />
		</div>
		...
		<div class="divTitle">
			<div class="divStatic">
				...
				<div class="divDesc">
					...
					<div class="divSend">
						<span><img src="../assets/time.png" /> 距离1.5km</span>
						<span><img src="../assets/money.png" /> 配送费6元</span>
						<span><img src="../assets/location.png" /> 预计时长12min</span>
					</div>
				</div>
			</div>
			<div class="divDesc">
			...
			<div class="divType">
				<ul>
					<li v-for="(item,index) in categoryList" :key="index"
						@click="categoryClick(index,item.id,item.type)" :class="{active:activeType === index}">
						{{item.name}}
					</li>
				</ul>
			</div>
						...
						<div>
							...
							<div class="divNum">
								<div class="divSubtract" v-if="item.number > 0">
									<img src="../assets/subtract.png" @click.prevent.stop="subtractCart(item)" />
								</div>
								<div class="divDishNum">{{item.number}}</div>
								<div class="divTypes" v-if="item.flavors && item.flavors.length > 0 && !item.number "
									@click.prevent.stop="chooseFlavorClick(item)">选择规格</div>
								<div class="divAdd" v-else>
									<img src="../assets/add.png" @click.prevent.stop="addCart(item)" />
								</div>
							</div>
						</div>
					</div>
				...
				<div v-for="item in cartData" :key="item.id" class="divCartItem">
					...
					<div class="divNum">
						<div class="divSubtract">
							<img src="../assets/subtract.png" @click="cartNumberSubtract(item)" />
						</div>
						<div class="divDishNum">{{item.number}}</div>
						<div class="divAdd">
							<img src="../assets/add.png" @click="cartNumAdd(item)" />
						</div>
					</div>
		...
		<van-dialog v-model="detailsDialog.show" :show-confirm-button="false" class="detailsDialog" ref="detailsDialog"
			...
			<div class="divNum">
				...
				<div class="right">
					<div class="divSubtract" v-if="detailsDialog.item.number > 0">
						<img src="../assets/subtract.png" @click="subtractCart(detailsDialog.item)" />
					</div>
					...
					<div class="divAdd" v-else>
						<img src="../assets/add.png" @click="addCart(detailsDialog.item)" />
					</div>
				</div>
			</div>
			...
			<div class="divNum">
				...
				<div class="right">
					<div class="divSubtract" v-if="setMealDialog.item.number > 0">
						<img src="../assets/subtract.png" @click="subtractCart(setMealDialog.item)" />
					</div>
					<div class="divDishNum">{{setMealDialog.item.number}}</div>
					<div class="divAdd" v-if="setMealDialog.item.number">
						<img src="../assets/add.png" @click="addCart(setMealDialog.item)" />
					</div>
		...
		methods: {
			...
			async subtractCart(item) {
				...
				if (res.data.code === 1) {
					this.dishList.forEach(dish => {
						if (dish.id === item.id) {
							dish.number = dish.number === 1 ? undefined : (res.data.data.number === 0 ?undefined : res.data.data.number)
						}
					})
					if (this.setMealDialog.show) {
						item.number = item.number === 1 ? undefined : (res.data.data.number === 0 ? undefined : res.data.data.number)
					}
					this.getCartData()
				} else {
					this.$notify({
...
<style scoped>
	...
	.divTitle {
		...
		background: #ffffff;
		border-radius: 6px;
		box-shadow: 0px 2px 5px 0px rgba(69, 69, 69, 0.1);
		position: absolute;
		left: 50%;
		top: 77px;
	...
	.divTitle .divStatic .divDesc .divName {
		width: 90px;
		height: 25px;
		opacity: 1;
		font-size: 18px;
	...
	.divTitle .divStatic .divDesc .divSend {
		opacity: 1;
		font-size: 11px;
		font-weight: 400;
		text-align: left;
	...
	.divTitle .divStatic .divDesc .divSend img {
		width: 14px;
		height: 14px;
		opacity: 1;
		vertical-align: sub;
	...
	.divTitle .divStatic .divDesc .divSend span {
		margin-right: 12px;
	}

	.divTitle .divStatic .divDesc .divSend span:last-child {
		margin-right: 0;
	}
